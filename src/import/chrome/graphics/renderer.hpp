...
namespace chrome::graphics {
    ...
    struct renderer {
        ...
        auto text_layout_boundary(
            std::string const& text, measure::point<float> const& top_left, std::string const& font_family,
            float const font_size, DWRITE_FONT_WEIGHT const font_weight = DWRITE_FONT_WEIGHT_NORMAL
		...
		auto render_dc() -> std::tuple<ID2D1Factory1*, ID2D1DeviceContext*, ID2D1DeviceContext*> {
			return std::make_tuple(_factory_d2d1.get(), _resource_device_context_d2d1.get(), _device_context_d2d1.get());
        }
    ...
    private:
        ...
        com::unique_ptr<ID2D1Factory1>           _factory_d2d1;
