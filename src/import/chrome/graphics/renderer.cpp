...
namespace chrome::graphics {
    ...
    renderer::renderer() {
        ...
        ID2D1Factory1* temporary_factory;
        ...
        hr = _factory_d2d1->CreateDevice(device_dxgi.get(), &temporary_device_d2d1);
            //D2D1CreateDevice(device_dxgi.get(), creation_properties, &temporary_device_d2d1);
        ...
        _resource_device_context_d2d1->SetAntialiasMode(D2D1_ANTIALIAS_MODE_ALIASED);
    ...
    auto renderer::begin_draw() -> void {
        ...
        _device_context_d2d1->SetAntialiasMode(D2D1_ANTIALIAS_MODE_ALIASED);
    ...
    auto renderer::draw_rectangle(measure::rectangle<float> const& fill_area, measure::color const& fill_color) -> void {
        ...
        _device_context_d2d1->DrawRectangle(D2D1::RectF(x, y, x + w, y + h), _brush.get());
    }
    ...
    auto renderer::text_layout_boundary(
        std::string const& text, measure::point<float> const& top_left, std::string const& font_family,
        float const font_size, DWRITE_FONT_WEIGHT const font_weight
        ...
        _factory_dwrite->CreateTextLayout(
            ...
            temporary_text_format, 5000.0f, 5000.0f, &temporary_text_layout
        );

        DWRITE_TEXT_METRICS textMetrics;
        DWRITE_OVERHANG_METRICS overhangMetrics;
        temporary_text_layout->GetMetrics(&textMetrics);

        auto text_layout = com::make_unique(temporary_text_layout);

        auto text_format = com::make_unique(temporary_text_format);
		return   measure::rectangle<float>{top_left.x, top_left.y, textMetrics.width, textMetrics.height};
    }
